...
class MovingPoint {
  ...
  adjustSizeBasedOnMouse() {
    let mouseDistandce = dist(mouseX, mouseY, this.x, this.y);
    if (mouseDistandce < this.reach) {
      let theSize = map(mouseDistandce, 0, this.reach, this.maxRadius, this.minRadius);
      this.radius = theSize;
    }
    else {
